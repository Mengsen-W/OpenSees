...
// $Source:
// /usr/local/cvs/OpenSees/SRC/material/uniaxial/TclModelBuilderUniaxialMaterialCommand.cpp,v
// $
...
int TclCommand_AxialSp(ClientData clientData, Tcl_Interp *interp, int argc,
                       TCL_Char **argv);

int TclCommand_AxialSpHD(ClientData clientData, Tcl_Interp *interp, int argc,
                         TCL_Char **argv);
...
int TclModelBuilderUniaxialMaterialCommand(ClientData clientData,
                                           Tcl_Interp *interp, int argc,
...
*/
  ...
  /*
    ...
    if (argc < 9) {
      opserr << "Warning insufficient arguments\n";
      printCommand(argc,argv);
      opserr << "Want: uniaxialMaterialSMA  tag  E  eps_L  sig_AS_s  sig_AS_f
  sig_SA_s  sig_SA_f" << endln;
    ...
    theMaterial = new SMAMaterial(tag, E, eps_L, sig_AS_s, sig_AS_f, sig_SA_s,
  sig_SA_f);
  ...
  */
  ...
  // ----- 1D J2 Plasticity ----
    ...
    // Fedeas
    theMaterial =
        TclModelBuilder_addFedeasMaterial(clientData, interp, argc, argv);

    // Drain
    if (theMaterial == 0)
      theMaterial =
          TclModelBuilder_addDrainMaterial(clientData, interp, argc, argv);

    // SNAP
    if (theMaterial == 0)
      theMaterial =
          TclModelBuilder_addSnapMaterial(clientData, interp, argc, argv);

    // Py, Tz, Qz models
    if (theMaterial == 0)
      theMaterial = TclModelBuilder_addPyTzQzMaterial(clientData, interp, argc,
                                                      argv, theDomain);
  ...
  if (theMaterial == 0) {
    ...
    if (matObject != 0) {
      ...
      if (theMaterial == 0) delete matObject;
    }
  }
  ...
  if (OPS_addUniaxialMaterial(theMaterial) == false) {
    ...
    delete theMaterial;  // invoke the material objects destructor, otherwise
                         // mem leak
    return TCL_ERROR;
  }
...
int TclCommand_KikuchiAikenLRB(ClientData clientData, Tcl_Interp *interp,
  ...
  if (argc < 11) {  // uniaxialMaterial KikuchiAikenLRB matTag? type? ar? hr?
                    // gr? ap? tp? alph? beta?
